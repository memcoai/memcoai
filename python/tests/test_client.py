...
from memco.client import Client, errors, types
from memco.client._auth import _merged
...
def test_auth_metadata_is_bearer_with_a_capital_b(client: Client, harness: Harness):
...
def test_auth_metadata_is_sent_on_every_method(client: Client, harness: Harness):
    client.memory.list_domains()
...
def test_caller_metadata_cannot_displace_the_credential(client: Client, harness: Harness):
...
def test_the_credential_is_withheld_from_the_health_probe(harness: Harness):
    ...
    with Client(token=TOKEN, host=harness.address, tls=False):
        pass
...
def test_not_serving_raises_unhealthy(harness: Harness):
    ...
    with pytest.raises(errors.MemcoUnhealthyError):
        Client(token=TOKEN, host=harness.address, tls=False)
...
def test_unreachable_server_raises_unavailable():
    with pytest.raises(errors.MemcoUnavailableError):
        Client(token=TOKEN, host="localhost:1", tls=False, timeout=2.0)


def test_check_health_false_skips_the_probe(harness: Harness):
    with Client(token=TOKEN, host=harness.address, tls=False, check_health=False):
        pass
    assert harness.health.checked_services == []
...
def test_verify_credentials_fires_list_domains(harness: Harness):
    with Client(token=TOKEN, host=harness.address, tls=False, verify_credentials=True):
        pass
...
def test_verify_credentials_surfaces_a_bad_token(harness: Harness):
    ...
    with pytest.raises(errors.MemcoAuthenticationError):
        Client(token=TOKEN, host=harness.address, tls=False, verify_credentials=True)
...
def test_server_errors_arrive_typed(client: Client, harness: Harness, code, expected):
...
def test_no_raw_grpc_error_escapes(client: Client, harness: Harness):
...
def test_oversized_query_never_reaches_the_server(client: Client, harness: Harness):
    with pytest.raises(errors.MemcoInvalidRequestError):
        client.memory.search("q" * 1001, domain="coding")
    assert harness.memory.calls == []


def test_missing_scope_never_reaches_the_server(client: Client, harness: Harness):
    with pytest.raises(errors.MemcoInvalidRequestError):
        client.memory.search("fine")
    assert harness.memory.calls == []


def test_enrich_requires_a_session(client: Client, harness: Harness):
    with pytest.raises(errors.MemcoInvalidRequestError):
...
def test_list_domains(client: Client, harness: Harness):
...
def test_start_session(client: Client):
    assert client.memory.start_session("coding").session_id == "session-a"


def test_search_sends_query_domain_and_tags(client: Client, harness: Harness):
...
def test_share_feedback(client: Client):
...
def test_revert_memory_reports_an_outcome_rather_than_raising(client: Client):
    result = client.memory.revert_memory("create-a")
    assert result.outcome is types.RevertOutcome.MERGED


def test_revert_not_found_is_a_value_not_an_exception(client: Client, harness: Harness):
...
def test_provenance_is_reachable_from_the_client(client: Client):
