...
GO_BANNER = "<!-- Code generated by gomarkdoc. DO NOT EDIT -->\n\n"
...
GO_PAGE = (
    ...
    "```\n"
    "\n"
    "Package memcoai is the Go SDK for Memco. It has more to say.\n"
    "\n"
    "Create a [Client](<#Client>) and read [Tool](<agent/index.md#Tool>).\n"
    "\n"
    "## Index\n"
    "\n"
    "- [type Client](<#Client>)\n"
    "\n"
    '<a name="Client"></a>\n'
    "## type [Client](<https://github.com/memcoai/memcoai/blob/main/go/memcoai/client.go#L1>)\n"
)
"""A package page in the shape gomarkdoc writes one."""

CANONICAL_GO = "https://docs.memco.ai/sdk/go/latest/"


def test_the_go_version_is_read_from_the_constant_the_tag_is_checked_against(script, tmp_path):
    version = tmp_path / "version.go"
    version.write_text(
        'package memcoai\n\n// Version is this release.\nconst Version = "1.2.3"\n', "utf-8"
    )
    assert script.go_version(version) == "1.2.3"
    # The release gate reads the constant, so a variable is not a version.
    version.write_text('package memcoai\n\nvar Version = "1.2.3"\n', "utf-8")
    with pytest.raises(SystemExit):
        script.go_version(version)


def test_the_go_markdown_tree_is_the_table_of_contents_root_package_first(script, tmp_path):
    # gomarkdoc writes one index.md per package, and the tree of them is all
    # the table of contents a Go reference has.
    markdown = write(
        tmp_path,
        {
            "zeta/index.md": "# zeta\n",
            "index.md": "# memcoai\n",
            "agent/index.md": "# agent\n",
            "agent/deep/index.md": "# deep\n",
            "agent/notes.md": "Not a package page.\n",
        },
    )
    assert script.go_packages(markdown) == [
        "index.md",
        "agent/deep/index.md",
        "agent/index.md",
        "zeta/index.md",
...
def test_a_go_reference_without_its_root_package_is_a_failure(script, tmp_path):
    markdown = write(tmp_path, {"agent/index.md": "# agent\n"})
    with pytest.raises(SystemExit):
        script.go_packages(markdown)


def test_an_angle_bracketed_destination_is_read_like_any_other(script):
    # gomarkdoc writes every destination in angle brackets, which the link
    # rewriter and the dangling-link check do not read. One that needs the
    # brackets, for a space or a parenthesis, keeps them: unwrapped, it would
    # stop being a link at all.
    text = (
        "A [Client](<#Client>), a [Tool](<agent/index.md#Tool>) and "
        "[ctx](<https://pkg.go.dev/context/#Context>).\n"
        "A [spaced](<a b.md>) and a [call](<f(1).md>) keep theirs.\n"
        "```md\n"
        "[fenced](<#Client>)\n"
        "```\n"
    )
    assert script.unangled(text) == (
        "A [Client](#Client), a [Tool](agent/index.md#Tool) and "
        "[ctx](https://pkg.go.dev/context/#Context).\n"
        "A [spaced](<a b.md>) and a [call](<f(1).md>) keep theirs.\n"
        "```md\n"
...
def test_gomarkdocs_generated_banner_does_not_open_the_page(script):
    assert script.heading(script.body(GO_PAGE)) == "memcoai"
    assert script.summarise(GO_PAGE) == "Package memcoai is the Go SDK for Memco."


@pytest.fixture
def go_ref(script, tmp_path, monkeypatch):
    """A two-package Go module, documented as doc2go and gomarkdoc build it."""
    root = tmp_path / "go"
    write(root, {"memcoai/version.go": 'package memcoai\n\nconst Version = "1.2.3"\n'})
    write(
    ...
    write(
        root / "docs" / "_build" / "markdown",
        {
            "index.md": GO_PAGE,
            "agent/index.md": GO_BANNER + "# agent\n\nPackage agent wires tools.\n\n"
            "Back to the [Client](<../index.md#Client>).\n",
        },
    )
    monkeypatch.setattr(script, "ROOT", tmp_path)
    return script.LANGUAGES["go"]()


def test_the_go_reference_lists_its_packages_under_one_group_root_first(script, go_ref):
    assert go_ref.title == "memcoai Go SDK 1.2.3"
    assert go_ref.summary == (
        "Go SDK for Memco Shared Memory. "
        "Install with `go get github.com/memcoai/memcoai/go/memcoai`."
    )
    assert [(page.title, page.group, page.href, page.source) for page in go_ref.pages] == [
        ("memcoai", "Packages", "index.html", "index.md"),
        ("agent", "Packages", "agent/index.html", "agent/index.md"),
    ]
    text = script.index(go_ref, CANONICAL_GO)
    assert "- [memcoai](index.md): Package memcoai is the Go SDK for Memco.\n" in text
    assert "- [agent](agent/index.md): Package agent wires tools.\n" in text


def test_the_go_reference_publishes_every_page_and_reaches_nothing_it_did_not(script, go_ref):
    # doc2go's assets are apparatus, not pages.
    assert script.unreachable(go_ref) == []
    pages = script.markdown_pages(go_ref)
    assert sorted(pages) == ["agent/index.md", "index.md"]
    full = script.full(go_ref, CANONICAL_GO)
    files = {"llms.txt": script.index(go_ref, CANONICAL_GO), "llms-full.txt": full, **pages}
    assert script.broken(go_ref.html, files) == []
    # gomarkdoc anchors its own symbols, so a page's markdown keeps fragments.
    assert "Create a [Client](#Client) and read [Tool](agent/index.md#Tool)." in pages["index.md"]
    assert "Back to the [Client](../index.md#Client)." in pages["agent/index.md"]
    assert pages["index.md"].startswith("# memcoai\n")
    # In one file, every link names the HTML page it lands on.
    assert "Create a [Client](index.html#Client) and read [Tool](agent/index.html#Tool)." in full
    assert "Back to the [Client](index.html#Client)." in full
    assert "Code generated" not in full
    assert "](<" not in full
