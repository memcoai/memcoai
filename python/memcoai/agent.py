...
def render(
    ...
    Every result carries :class:`~memcoai.types.Instructions` — the service's own
...
def briefing(domain: DomainEntry, instructions: Instructions) -> str:
    ...
    Every word of this comes from :meth:`~memcoai.operations.MemoryOperations.list_domains`
...
def _reference(target: str) -> str:
    ...
    Args:
        target: The role's target, such as ``memcoai.types.DataSource.AGENT``.
...
def _describe(method: Callable[..., object], name: str) -> tuple[str, dict[str, str]]:
    ...
    arrives with every export in ``memcoai/memory/tools.json``, the same copy the
...
def _fields(cls: type) -> dict[str, tuple[type, bool]]:
    ...
    :class:`~memcoai.types.Tag` or :class:`~memcoai.types.FeedbackRating` reaches
