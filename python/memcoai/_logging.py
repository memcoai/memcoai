...
Everything this SDK logs goes to a logger under ``memcoai``: each module writes to
``logging.getLogger(__name__)``, which makes ``memcoai._sync``, ``memcoai._channel``
and the rest children of one tree. Configuring ``memcoai`` therefore governs all
...
``memcoai`` logger itself, which leaves the records propagating as usual. Both
...
class _MemcoStreamHandler(logging.StreamHandler):  # type: ignore[type-arg]
    ...
    constructed, which here is during ``import memcoai``. A process that
...
def configure(env: Mapping[str, str] | None = None) -> None:
    ...
    Called once when :mod:`memcoai` is imported. Safe to call again: the SDK's own
    ...
    try:
        ...
        # this runs during `import memcoai`, so under `-W error` the warning
