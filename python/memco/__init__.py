...
``memco._channel`` and so on — so configuring ``memco`` governs all of it::

    logging.getLogger("memco").setLevel(logging.DEBUG)
...
own handler off with ``log_level="none"`` and set the level on the ``memco``
